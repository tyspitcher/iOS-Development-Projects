//
//  ProfilePhotoSourcePicker.swift
//  ThreadShare
//
...
struct ProfilePhotoSourcePicker: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Button {
                ...
                Label(previewImageData == nil ? "Choose Profile Photo" : "Change Profile Photo", systemImage: "camera.viewfinder")
            ...
            if let previewImageData {
                DataBackedImageView(data: previewImageData, contentMode: .fill)
                    .frame(width: 124, height: 124)
                    .clipShape(Circle())
                    .overlay(Circle().stroke(AppTheme.border, lineWidth: 1))
            }

            Text("Choose a photo from your library or take one with your camera.")
        ...
        .confirmationDialog("Choose Photo Source", isPresented: $isShowingSourceChooser, titleVisibility: .visible) {
            ...
            Text("ThreadShare can use the camera or your photo library for your profile photo.")
        }
        .photosPicker(
            ...
            Task {
                do {
                    if let data = try await newItem.loadTransferable(type: Data.self) {
                        ...
                        onError("Could not load the selected profile photo.")
                    }
                } catch {
                    onError("Could not load the selected profile photo.")
                }
