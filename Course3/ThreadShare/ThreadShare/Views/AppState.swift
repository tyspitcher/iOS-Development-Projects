...
final class AppState: ObservableObject {
    ...
    var visibleItems: [ThreadItem] {
        filteredItems
    }

    var followedUsers: [UserProfile] {
        users.filter(\.isFollowedByCurrentUser)
    }
    ...
    var hasFriendRequestActivity: Bool {
        !outgoingFriendRequestUserIDs.isEmpty || !incomingFriendRequestUserIDs.isEmpty
    }

    var pendingBorrowRequests: [BorrowRequest] {
        borrowRequests.filter { $0.status == .pending }
    }
    ...
    func isCurrentUser(_ user: UserProfile) -> Bool {
    ...
    func requestFriend(for userID: UserProfile.ID) {
        // Maintained for older callsites; now behaves as sending a request.
    ...
    private func persistFriendRequestState() {
        let state = FriendRequestState(
            outgoingUserIDs: outgoingFriendRequestUserIDs,
            incomingUserIDs: incomingFriendRequestUserIDs
        )
        Task {
            try? await repository.saveFriendRequestState(state)
        }
    }
}
