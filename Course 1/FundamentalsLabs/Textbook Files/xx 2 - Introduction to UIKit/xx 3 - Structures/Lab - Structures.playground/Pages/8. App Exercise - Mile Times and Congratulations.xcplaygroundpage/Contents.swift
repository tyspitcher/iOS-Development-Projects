...
struct RunningWorkout {
    ...
    var averageMileTime : Double {
        (distance / 1600) / (time / 60) //assuming distance in meters
                                        //and time is in minutes
    }
}
var recentWorkout = RunningWorkout(distance: 1600, time: 6, elevation: 88)
print(recentWorkout.averageMileTime)
/*:
...
struct Steps {
    var steps: Int {
        willSet {
            if newValue == goal {
                print("You hit your goal of \(goal) steps! Great effort!")
            }
        }
    }
    var goal: Int
...
var stepsToday = Steps(steps: 9999, goal: 10000)
stepsToday.takeStep()
